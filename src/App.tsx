...
import { ImportTasksView } from "./components/ImportTasksView";
...
export default function App() {
  ...
  const [isAddingTaskForDate, setIsAddingTaskForDate] = useState<string | null>(
    null,
  );
  ...
  const googleTasksLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        const scheduleToExport =
          activeTab === "today"
            ? todaySchedule
            : getResolvedSchedule(new Date(selectedUpcomingDate));
        const dateToExport =
          activeTab === "today" ? currentDateStr : selectedUpcomingDate;

        const { successCount, errorCount } = await exportToGoogleTasks(
        ...
        if (errorCount > 0) {
          showNotification(
            `Berhasil mengekspor ${successCount} tugas. ${errorCount} gagal.`,
          );
        } else {
          showNotification(
            `Berhasil mengekspor ${successCount} tugas ke Google Tasks!`,
          );
        }
      } catch (err) {
  ...
  const activeItem =
    activeItemIndex >= 0 ? todaySchedule[activeItemIndex] : null;
  ...
  const handleDownloadApp = async () => {
    if (window.deferredPrompt) {
      ...
      alert(
        "Untuk menginstall app ini, silakan gunakan fitur 'Add to Home Screen' atau 'Install App' di menu browser Anda (biasanya terletak di titik tiga pojok kanan atas atau icon share di iOS). Pada beberapa browser tertentu, Anda mungkin juga perlu memastikan telah menerima cookie.",
      );
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans selection:bg-red-100 selection:text-red-900 flex flex-col overflow-x-hidden">
      <header className="bg-gradient-to-r from-emerald-600 via-teal-500 to-cyan-600 text-white py-4 px-6 sticky top-0 z-50 shadow-lg border-b border-black/10">
        <div className="max-w-7xl mx-auto flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <button
              onClick={() => {
                if (!isAudioEnabled) {
                  ...
                  setTimeout(() => {
                    speakText(
                      "Sistem alarm pengingat stasiun telah diaktifkan.",
                      volume,
                    );
                  }, 3500);
                }
          ...
          <AuthMenu
            ...
            onImportSuccess={reloadLocalData}
            onOpenImportTasks={() => setActiveTab("import-tasks")}
          />
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-4">
          <div className="flex flex-wrap items-center gap-1 sm:gap-2 border-b border-white/20 pb-px mt-4">
            ...
            >
              <CalendarDays className="w-3.5 h-3.5 sm:w-4 sm:h-4 shrink-0" />6
              Hari Kedepan
            </button>
            <button
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
                <div className="flex items-center justify-between mb-6">
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="flex flex-col gap-2">
                      <div className="flex flex-wrap gap-2">
                        <button
                          onClick={() =>
                            downloadICS(todaySchedule, currentDateStr)
                          }
                        ...
                        <button
                          onClick={() => {
                            if (!user) {
                              alert(
                                "Harap login terlebih dahulu untuk menambah aktivitas",
                              );
                              return;
                            }
                      ...
                      <div className="flex">
                        <button
                          onClick={async () => {
                            ...
                            if (
                              window.confirm(
                                "Yakin ingin menghapus semua task hari ini?",
                              )
                            ) {
                              await deleteAllScheduleItems(currentDateStr);
                              showNotification(
                                "Semua task hari ini berhasil dihapus",
                              );
                            }
                          }}
                  ...
                  <div className="text-sm px-3 py-1 bg-gray-100 rounded-full font-medium text-gray-600 shrink-0">
                    {Object.keys(progress).filter((k) => progress[k]).length} /{" "}
                    {todaySchedule.length} Selesai
                  </div>
                </div>
                      ...
                      >
                        ...
                        <div
                          className={cn(
                            ...
                            isActive
                              ? "bg-emerald-500 border-emerald-100 z-10 animate-pulse"
                              : isCompleted
                                ? "bg-emerald-500 border-white z-10"
                          ...
                          className={cn(
                            ...
                            isActive
                              ...
                              : isCompleted
                                ...
                                : "bg-white border-gray-100/50 hover:border-gray-300",
                          )}
                        ...
                        >
                              ...
                              {user && (
                                <button
                                  onClick={(e) => {
                                    ...
                                    setEditingTask({
                                      item,
                                      index: todaySchedule.indexOf(item),
                          ...
                          <p className="text-sm font-bold text-gray-500 mt-1 line-clamp-2">
                            {item.notes}
                          </p>
                        </div>
                      </div>
              ...
              <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100 sticky top-24">
                ...
                {activeItem ? (
                  ...
                  >
                    ...
                    <p className="font-bold text-gray-600 text-sm mb-4">
                      {activeItem.notes}
                    </p>
                  ...
                  <div className="p-5 rounded-xl bg-gray-50 border border-gray-200 text-center">
                    <p className="text-gray-500 text-sm">
                      Tidak ada jadwal yang aktif saat ini.
                    </p>
                  </div>
                )}
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <h3 className="font-semibold text-gray-900">
                    Tetap Dapat Notifikasi Meskipun Web Ditutup!
                  </h3>
                  <p className="text-sm text-gray-600 mt-1">
                    Pilih tanggal di bawah ini, lalu klik kartu jadwal pada hari
                    tersebut untuk men-download file Kalender (.ics). Sinkronkan
                    dengan <b>Google Calendar / Kalender iPhone</b> Anda agar
                    alarm berbunyi 100% tanpa jaringan internet dan saat app
                    ditutup.
                  </p>
                </div>
              </div>
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 border-b border-indigo-100 pb-4">
                    ...
                    <div className="flex flex-col sm:items-end gap-2 w-full sm:w-auto">
                      <div className="flex flex-col sm:flex-row flex-wrap gap-2 w-full sm:w-auto">
                        <button
                          onClick={() =>
                            downloadICS(schedule, selectedUpcomingDate)
                          }
                      ...
                      <div className="flex w-full sm:w-auto justify-end">
                        <button
                          onClick={async () => {
                            ...
                            if (
                              window.confirm(
                                "Yakin ingin menghapus semua task pada tanggal ini?",
                              )
                            ) {
                    ...
                    {schedule.map((item, index) => (
                      <div
                        ...
                        className={cn(
                          ...
                          item.isBreak
                            ? "bg-white/50 border-red-100 hover:border-red-200"
                            : "bg-white border-indigo-100 hover:border-indigo-300 hover:bg-indigo-50/80",
                        )}
                      >
                        ...
                        <h3
                          className={cn(
                            ...
                            item.isBreak
                              ? "text-[#c0392b]"
                              : "text-gray-900 group-hover:text-emerald-800 transition-colors",
                          )}
                        >
                          {item.activity}
                        </h3>
        ...
        {activeTab === "analytics" &&
          (() => {
            const totalMinutes = weekPieData.reduce(
                  ...
                  {weekPieData.length > 0 && (
                    <div className="mt-4 bg-emerald-50 px-5 py-2.5 rounded-full border border-emerald-100 shadow-sm">
                      <span className="text-emerald-800 font-semibold text-sm">
                        Total Waktu:{" "}
                        <span className="font-bold">{totalDurationStr}</span>
                      </span>
                    </div>
                  )}
                  {weekPieData.length === 0 && (
                      ...
                      <div className="flex flex-col gap-4">
                        {weekPieData.map((item, index) => {
                          ...
                          return (
                            <div
                              key={index}
                              className="flex flex-col gap-3 pb-4 border-b border-gray-100 last:border-0 last:pb-0"
                            >
                              <div className="flex items-center justify-between">
                                <div className="flex items-center gap-3 sm:gap-4">
                                  ...
                                  <div className="flex flex-col">
                                    ...
                                    <div className="flex items-center gap-2">
                                      <span className="font-mono text-[10px] sm:text-xs text-gray-500 font-medium">
                                        {percentage}% dari total waktu
                                      </span>
                                    </div>
                                  </div>
                                </div>
      ...
      {isAddingTaskForDate && (
        <AddActivityModal
          ...
          onSave={async (newItem) => {
            ...
            const totalScheduledMinutes = currentWeekAnalytics.reduce(
              (sum, curr) =>
                curr.name !== "Waktu Kosong (Tak Terjadwal)"
                  ? sum + curr.value
                  : sum,
              0,
            );
            ...
            if (totalScheduledMinutes + newItem.duration > maxWeeklyMinutes) {
              alert(
                `Gagal menambah aktivitas: Total jadwal Anda dalam seminggu melebihi 168 jam (Anda sudah memiliki ${Math.floor(totalScheduledMinutes / 60)} jam ${totalScheduledMinutes % 60} menit). Silakan kurangi durasi aktivitas lain terlebih dahulu.`,
              );
              return;
            }
      ...
      {activeItem && showActivePopup && (
        <div className="fixed bottom-4 left-4 right-4 md:left-auto md:right-8 md:bottom-8 md:w-[28rem] lg:w-[40rem] xl:w-[50rem] bg-white rounded-3xl shadow-2xl border border-gray-200 z-50 animate-in slide-in-from-bottom-5 fade-in duration-300 overflow-hidden">
          ...
          >
            <div className="flex items-center gap-3 md:gap-4 mb-4 md:mb-4 pr-20 md:pr-0">
              <Activity
                className={cn(
                  ...
                  activeItem.isBreak ? "text-[#c0392b]" : "text-emerald-600",
                )}
              />
            ...
            <h3 className="font-bold text-gray-900 line-clamp-3 pr-2 md:pr-6 text-4xl sm:text-5xl md:text-4xl lg:text-6xl leading-tight md:leading-tight lg:leading-tight mb-4 md:mb-0">
              {activeItem.activity}
            </h3>
            ...
            {remainingSeconds !== null && (
              <div className="mt-6 md:mt-8 lg:mt-12 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="text-2xl sm:text-3xl md:text-xl lg:text-3xl text-gray-500 font-medium flex items-center gap-2 md:gap-2">
                  <Timer className="w-8 h-8 sm:w-10 sm:h-10 md:w-6 md:h-6 lg:w-8 lg:h-8" />{" "}
                  Sisa Waktu:
                </div>
                <div
                  className={cn(
                    ...
                    activeItem.isBreak ? "text-[#c0392b]" : "text-emerald-700",
                  )}
                >
                  {formatRemainingTime(remainingSeconds)}
                </div>
      ...
      {showPermissionGuide && (
        <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="bg-white rounded-3xl shadow-2xl max-w-sm w-full p-6 relative animate-in zoom-in-95 duration-300 flex flex-col max-h-[90vh]">
            ...
            <h2 className="text-xl font-bold text-gray-900 mb-2 shrink-0">
              Panduan Penggunaan
            </h2>
            <div className="text-sm text-gray-600 space-y-4 mb-6 overflow-y-auto pr-2 flex-grow">
              <p>
                Agar alarm peringatan (suara & pop-up) dapat berjalan otomatis,
                pilih salah satu dari 2 metode berikut:
              </p>
              <ul className="list-disc pl-5 space-y-3">
                <li>
                  <strong>
                    Metode 1: Biarkan Tab Terbuka (Suara Kustom & Suara Bicara)
                  </strong>
                  <br />
                  Aplikasi web tidak dapat membunyikan suara khusus jika Anda
                  menutup tab. Pastikan{" "}
                  <b>tab tetap terbuka di latar belakang</b>, tombol{" "}
                  <b>Audio ON</b> aktif, dan Izinkan Notifikasi jika Anda ingin
                  mendengar peringatan dengan suara wanita yang berbicara.
                </li>
                <li>
                  <strong>
                    Metode 2: Alarm Meskipun Web Ditutup (Rekomendasi)
                  </strong>
                  <br />
                  Klik tombol{" "}
                  <b>
                    <span className="text-emerald-600 bg-emerald-50 px-1 rounded inline-flex items-center gap-1 border border-emerald-200">
                      <CalendarPlus className="w-3 h-3" /> Sinkronkan Alarm ke
                      Kalender HP
                    </span>
                  </b>
                  . Alarm akan otomatis masuk ke Aplikasi Kalender HP Anda,
                  dijamin berbunyi 100% meskipun web ditutup.{" "}
                  <i>
                    Catatan: Suara alarm ini akan menggunakan suara notifikasi
                    standar bawaan HP Anda.
                  </i>
                </li>
              </ul>
              <p className="text-xs bg-yellow-50 text-yellow-800 p-3 rounded-xl border border-yellow-200 leading-relaxed font-medium">
                *Catatan: Sistem Android / iOS otomatis "membunuh" aplikasi web
                di latar belakang demi menghemat baterai. Metode Export Kalender
                (Metode 2) sangat direkomendasikan agar jadwal tetap aman!
              </p>
            </div>
            <button
              onClick={() => setShowPermissionGuide(false)}
            ...
            <div className="overflow-y-auto flex-grow pr-2 space-y-6">
              <div>
                <h3 className="text-sm font-semibold text-gray-900 mb-2">
                  Teks Notifikasi Saat Sesi Dimulai
                </h3>
                <p className="text-xs text-gray-500 mb-2">
                  Gunakan <code>{`{{activity}}`}</code> untuk nama aktivitas,
                  dan <code>{`{{duration}}`}</code> untuk durasi (menit).
                </p>
                <textarea
              ...
              <div>
                <h3 className="text-sm font-semibold text-gray-900 mb-2">
                  Teks Notifikasi Pertengahan Sesi
                </h3>
                <p className="text-xs text-gray-500 mb-2">
                  Gunakan <code>{`{{activity}}`}</code> untuk nama aktivitas.
                </p>
              ...
              <div>
                <h3 className="text-sm font-semibold text-gray-900 mb-2">
                  Teks Notifikasi Sesi Akan Berakhir
                </h3>
                <p className="text-xs text-gray-500 mb-2">
                  Gunakan <code>{`{{activity}}`}</code> untuk nama aktivitas,
                  dan <code>{`{{nextActivity}}`}</code> untuk nama aktivitas
                  selanjutnya.
                </p>
                <textarea
                  className="w-full text-sm p-3 border border-gray-200 rounded-xl bg-gray-50 focus:bg-white transition-colors"
      ...
      <footer className="bg-gray-900 border-t border-gray-800 text-gray-400 py-8 text-center mt-auto">
        <div className="max-w-7xl mx-auto px-4 flex flex-col items-center gap-4">
          <p>
            &copy; {new Date().getFullYear()} GROW UP | Habit Changer. All
            rights reserved.
          </p>
